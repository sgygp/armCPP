...
void Adc32::init(int xHz,u8 iWhat) {

	ADC_InitTypeDef ADC_InitStructure;
	GPIO_InitTypeDef GPIO_InitStructure;

	RCC_APB2PeriphClockCmd( RCC_APB2Periph_ADC1, ENABLE); //使能ADC1,GPIOA时钟
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; //设置GPIO口速度为50MHz
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;	  //模拟输入
	GPIO_Init(GPIOB, &GPIO_InitStructure);			  //初始化GPIO_B1
	...
	if(iWhat==1)
	{
		...
		ADC_RegularChannelConfig(ADC1, ADC_Channel_9,  1, ADC_SampleTime_239Cycles5); //配置ADC1的通道
	else
