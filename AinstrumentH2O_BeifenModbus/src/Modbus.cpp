...
bool Modbus::run(u32 ms,u8 addr,u32 zhouqi,u16 jicunqiStart,u8 geshu) { //5ms运行一次
	if (ms % 100 == 0) //每100毫秒执行一次指令
		DoCommand();
	if((ms%zhouqi)==0)
		this->putReadCommand(addr,jicunqiStart,geshu);
//	if((ms%(zhouqi*3))==(zhouqi/2))
//		this->putReadCommand(addr,0x06,2);
...
void Modbus::DoCommand() {
	...
	memcpy(m_com->m_data->m_FA, m_commandBuf[m_pR] + 1, len);
	m_pR = (m_pR + 1) % 10;
...
bool Modbus::runMain() {
	...
	if (buf[1] == 0x03||buf[1] == 0x02) { //读取 北分的通讯返回的是02（不合法查询数据地址），但是不影响使用
...
//	CRC16::JisuanCRC(buf, 6, true);
	memcpy(temp,m_com->m_data->m_SHOU,8);
	putCommand(8);
