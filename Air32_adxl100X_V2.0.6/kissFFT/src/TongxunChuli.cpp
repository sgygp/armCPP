...
bool TongxunChuli::run(float batt) {
	...
	{
		int addr=RtcBkp::getBkpData(BKP_DR2)-shengyuSuduJiasuducishu;

		int benci;
		if(shengyuSuduJiasuducishu>_LEN_)
		{
			benci=_LEN_;
			shengyuSuduJiasuducishu-=_LEN_;
		}else
		{
		...
		{
			m_25->SPI_FLASH_PageReadData((addr+i)*256,100);
			if(AorS<10)
				shuju[i]=*(u16*)&m_25->spi1_tx[4+AorS*2];
			else
			{
				shuju[i]=*(long*)&m_25->spi1_tx[4+10*2+(AorS-10)*4];
	...
	if(x==1&&buf[0]=='?')
	{
		...
		Shuju::TianjiaShuju(tt,54+2,RtcBkp::getBkpData(BKP_DR3),4);
		for(int i=0;i<8;i++)
			Shuju::TianjiaShuju(tt,61+2+5*i,RtcBkp::getBkpData(BKP_DR35+4*i),4);
		memcpy(m_com->m_data->m_FA,tt,102);
		m_com->FasongX(102);
		...
		{
			if((x==2||x==4)&&Shuju::isNumber(buf[1]))
			{
				u8 a=buf[1]-'0';
				shengyuSuduJiasuducishu=RtcBkp::getBkpData(BKP_DR2);
				AorS=10+a*2;//傅里叶频率
			}
		...
		}else 	if((x==1||x==3)&&(buf[0]=='b'||buf[0]=='B'))
		{
			shengyuSuduJiasuducishu=RtcBkp::getBkpData(BKP_DR2);
			AorS=9;//幅值和
		}else 	if((x==2||x==4)&&(buf[0]=='u'||buf[0]=='U')&&Shuju::isNumber(buf[1]))
		{
			u8 a=buf[1]-'0';
			shengyuSuduJiasuducishu=RtcBkp::getBkpData(BKP_DR2);
			AorS=11+2*a;//傅里叶赋值
		...
		}else if(((buf[0]=='p'||buf[0]=='P')))//
		{
			if((x==5||x==7)&&Shuju::isNumber(buf[1])&&Shuju::isNumber(buf[2])&&Shuju::isNumber(buf[3])&&Shuju::isNumber(buf[4]))
			{
				...
				RtcBkp::setBkpData(BKP_DR3,aa);
				m_com->m_data->m_FA[0]='P';
			...
			{
				m_com->m_data->m_FA[0]='P';
				int k=RtcBkp::getBkpData(BKP_DR3);
			...
			}else	if((buf[1]=='O'||buf[1]=='o')&&(buf[2]=='W'||buf[2]=='w')){
				m_com->m_data->m_FA[0]='P';
				m_com->m_data->m_FA[1]='O';
				m_com->m_data->m_FA[2]='W';
				int k=batt;
				for(int i=3;i<7;i++)
				{
					m_com->m_data->m_FA[9-i]=k%10+'0';k/=10;
				}
