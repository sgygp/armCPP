...
void Screen::init() {
	...
	OLED_ShowWave();
	OLED_Refresh();
	m_curWeizhi=RtcBkp::getBkpData(BKP_DR2)-1;
	m_page=0;
	m_25->readShuju(m_curWeizhi-m_page);
}
...
void Screen::show() {
	...
	if(m_once)
	{
		...
		OLED_ShowChar(0,3*8,'0'+m_page,8,1);
		OLED_Refresh();
		m_once=false;
	}
}
...
void Screen::Bianhuan(long* shu, u8* xiaoshu) {
