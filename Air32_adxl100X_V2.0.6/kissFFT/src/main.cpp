/*版本说明
  2.0.5 更改了V7000,当大于6553时数据溢出的问题
  2.0.5 当电池电压低于设定值时停止采样。P?读取设定值，P3400设置3400mV停止采样，POW读取当前电压值
  2.0.4 修改了第一次数据采集后，显示总是9999的问题（存储位置修改）
  2.0.3 修改了屏幕显示次数大于128次后不正常的问题。调整了通讯数据次数的问题。
...
int main(int argc, char* argv[])//开发板，8MHz电流2.9mA，72MHz电流11mA，256MHz电流31mA
{
	...
	for(int i=0;i<AD_LEN;i++)
		batt+=Adc32::data_table[i];
	batt/=AD_LEN;
	batt=batt*3915.0/3232.0;
 	...
 	if(bPingmu)
	{
		...
		while(1){
			...
			if (Timer::m_bMS5) {
				...
				if(Anjian::isKey1()||ms>50000)//按下关机键或者无操作LED按键半分钟后关机
					break;
			}
	...
	{
		if(batt<RtcBkp::getBkpData(BKP_DR3))
		{
			m_wake.setOFF();
			Sleep::sleep();
		}
