...
u16 W25X16::jiasudu_sudu[50]={0};


#define SUDUGAOPIN 1//启用此行使用速度高频幅度，注释掉用加速度高频幅值
...
void W25X16::WaitForWriteEnd() {
	...
	for (int i=0; i < 10000000; i++) {                  //整片擦除等待最长40秒
...
void W25X16::SPI_FLASH_BlockErase(u32 SectorAddr) {//64K 块区擦除
	spi1_tx[0] = W25X_WriteEnable;
	SPI_Write(1, 0);
	//	WaitForWriteEnd();
...
int W25X16::JisuanJiasuduSudu(int weizhi)//返回峰个数
{
	...
	float xishu=9.8*k/1000.0/(YUANMA_1G);//单位m/s2
	...
	/*	if(zuidaX>1)
	{
		float xishu1=1000/caiyangpinlv;//单位毫米/秒
	...
	if(shangshengX>1)
	{
		float xishu2=1000.0/caiyangpinlv;//单位um
...
void W25X16::WriteBenciSudu() {
	...
	jiasudu_sudu[0]=jiasuduHe*1.41421356*10;//有效值乘以根2
	...
	float t=RtcBkp::getBkpData(BKP_DR21);
	if(t>9999)//||t<0.1
		t=1000;
	jiasudu_sudu[1]*=(t/1000.0);
	t=RtcBkp::getBkpData(BKP_DR22);
		if(t>9999)//||t<0.1
			t=1000;
	jiasudu_sudu[2]*=(t/1000.0);
	...
	memcpy(spi1_tx+4,jiasudu_sudu,sizeof(jiasudu_sudu));
	SPI_FLASH_PageWriteData(zongcishu*256,sizeof(jiasudu_sudu));

	readShuju(zongcishu);

}

void W25X16::readShuju( int cishu) {
	SPI_FLASH_PageReadData(cishu*256,sizeof(jiasudu_sudu));
	memcpy(jiasudu_sudu,spi1_tx+4,sizeof(jiasudu_sudu));
}
...
void W25X16::jisuanFFT() {
	...
	float xishu=9.8*k/1000.0/(YUANMA_1G);//单位m/s2
//	float xishu=9.8/(YUANMA_1G*k/1000.0);//单位m/s2
	for(int i=0;i<nFFT;i++)
	{
		shuju[i]-=pingjunJiasudu;
		shuju[i]*=xishu;
...
void W25X16::getJieguo(u16 *jieguo) {
	...
	for(int j=0;j<8;j++)
	{
		...
		ll[2*j]=pinlv*10;//保留了1位小数
		ll[2*j+1]=max*1000;//乘以100，保留了1位小数
	}
