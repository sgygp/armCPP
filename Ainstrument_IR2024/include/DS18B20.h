...
public:
	DS18B20(u32 rcc, GPIO_TypeDef *port, u16 pin, u8 m_geshu);
	void init(void);
	void get_Temp(void);
private:
	u32 rcc;
	...
	u8 *temp_buff; //存储读取的温度数组9字节，ID数组8字节
	u8 *tempBit;
	u8 *tempU8;
	u8 *error;
private:
	void DS18B20_DATA_OUT(bool a) {
		for (int i = 0; i < m_geshu; i++) {
			if (a)
				GPIO_SetBits(port, pin << i);
			else
				GPIO_ResetBits(port, pin << i);
		}
	}
	u8 DS18B20_DATA_IN(u16 ppin) {
