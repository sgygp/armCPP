...
*/
	...
	if (fabs(a-m_ad[0]) > 30||fabs(b-m_ad[1])>30)//变化超过10%，快速显示
		lvbo = 2;
	else if (lvbo < LVBO)
		lvbo++;
	...
	u8 LBOCISHU=5;
	float zhi;
//	if(m_buchang[1]>0.1)//补偿2 *AD0-AD1
//		zhi=m_ad[0]*m_buchang[1]-m_ad[1];
//	else
		zhi=m_ad[0]/m_ad[1];
	m_zhiLVBO[m_pZhilvbo]=zhi*(1+ (wendu - m_wenduBiao) * m_buchang[0]/100.0);
	m_pZhilvbo=(m_pZhilvbo+1)%LBOCISHU;
	for (int i = 0; i < LBOCISHU; i++)
		ss+=m_zhiLVBO[i];
