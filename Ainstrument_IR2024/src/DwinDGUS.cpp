...
void DwinDGUS::FasongX() {
	u8 geshu = m_USART1_FA[2] + 3;
	if (geshu > 0) {
		DMA1_USART1_TX(geshu);
		for (int i = 0; i < 100; i++) {
			delay_ms(1);
			if (m_bOK)
				break;
...
void DwinDGUS::FillRect(u16 dizhi, u16 x1, u16 y1, u16 x2, u16 y2, u16 color) {
	...
	m_USART1_FA[2] = 0x11;
	m_USART1_FA[3] = 0x82;
	m_USART1_FA[4] = dizhi >> 8;
	m_USART1_FA[5] = dizhi; //变量地址
	m_USART1_FA[6] = 0x00;
	m_USART1_FA[7] = 0x04;
	m_USART1_FA[8] = 0x00;
	m_USART1_FA[9] = 0x01;
	...
	m_USART1_FA[16] = y2 >> 8;
	m_USART1_FA[17] = y2;
	m_USART1_FA[18] = color >> 8; //颜色0xffff为反色
	m_USART1_FA[19] = color;
...
void DwinDGUS::ShowTubiao(u16 x1, u16 y1, u16 x2, u16 y2, u8 picture) {
	...
	m_USART1_FA[3] = 0x82;
	m_USART1_FA[4] = 0x00;
	m_USART1_FA[5] = 0x70; //变量地址
	m_USART1_FA[6] = 0x00;
	m_USART1_FA[7] = 0x06; //剪切粘贴图形
	m_USART1_FA[8] = 0x00;
	m_USART1_FA[9] = 0x01; //指令数
	m_USART1_FA[10] = 0x00;
	m_USART1_FA[11] = picture;

	m_USART1_FA[3 + 9] = x1 >> 8;
	m_USART1_FA[4 + 9] = x1;
	m_USART1_FA[5 + 9] = y1 >> 8;
	m_USART1_FA[6 + 9] = y1;
	m_USART1_FA[7 + 9] = x2 >> 8;
	m_USART1_FA[8 + 9] = x2;
	m_USART1_FA[9 + 9] = y2 >> 8;
	m_USART1_FA[10 + 9] = y2;
	m_USART1_FA[20] = x1 >> 8;
	m_USART1_FA[21] = x1;
	m_USART1_FA[22] = y1 >> 8;
...
void DwinDGUS::SetMiaoshu_Ziku(u16 dizhi, u8 a, u8 b, u8 dot) {
	...
	m_USART1_FA[7] = b;
	m_USART1_FA[8] = 0; //左对齐
	m_USART1_FA[9] = 4 - dot; //整数位数
	m_USART1_FA[10] = dot; //小数位数
	m_USART1_FA[11] = 5; //数据类型 U16
	FasongX();
}
...
u32 DwinDGUS::ReadDGUS() {
	...
	if (!m_USART1_shouPos)
		return 0;
	dd = (Timer::ms5_jishu + Timer::MAXSHU - m_USART1_curShou_ms);
	dd = dd % Timer::MAXSHU;
	if (dd < SHOUKONGXIAN)
		return 0;

	dd = 0;
	if (m_USART1_shouPos == 9 && m_USART1_SHOU[0] == 0x5A
			&& m_USART1_SHOU[1] == 0xA5) {
		...
		dd <<= 8;
		dd |= m_USART1_SHOU[8];
	} else if (m_USART1_shouPos == 11 && m_USART1_SHOU[0] == 0x5A
			&& m_USART1_SHOU[1] == 0xA5) {
		//////////////返回值前2个字节代表地址，后两个字节代表数据值
		dd = m_USART1_SHOU[5]; //取变量地址的低8位
		dd <<= 8;
		dd |= m_USART1_SHOU[8];
		dd <<= 8;
		dd |= m_USART1_SHOU[9];
...
void DwinDGUS::SetDGUS_Dizhi(u16 dizhi, char buf[], u8 geshu) {
	...
	m_USART1_FA[2] = geshu + 3;
	m_USART1_FA[3] = 0x82;
	m_USART1_FA[4] = dizhi >> 8;
	m_USART1_FA[5] = dizhi;
	for (i = 0; i < geshu; i++)
		m_USART1_FA[6 + i] = buf[i];
	FasongX();
}
...
void DwinDGUS::SendQuxian(u16 *m_data, u8 quxianNo, u8 geshu) {
	...
	m_USART1_FA[2] = 2 + geshu * 2;
	m_USART1_FA[3] = 0x84;
	m_USART1_FA[4] = quxianNo;
	for (int i = 0; i < geshu; i++) {
		m_USART1_FA[5 + i * 2] = m_data[i] >> 8;
		m_USART1_FA[6 + i * 2] = m_data[i];
