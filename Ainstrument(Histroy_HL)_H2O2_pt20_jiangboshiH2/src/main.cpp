...
//	HistoryData m_history3(&ainstr3);//曲线初始化为0
	...
	WatchDog dog;
	dog.init(IWDG_Prescaler_16, 4000); //
...
//	COM m_com;
//	m_com.init();
//	Modbus m_modbus(&m_com,0);//串口里面就定义了一个，所以必须是0
...
//				AinstrRun(ainstr1, m_ziwai.m_shuju.m_nongdu[0], m_ziwai.m_shuju.m_xishu[0], m_ziwai.m_shuju.m_yali); //0为恒温温度，1为探测器温度
//				AinstrRun(ainstr2, m_ziwai.m_shuju.m_nongdu[1], m_ziwai.m_shuju.m_xishu[1], m_ziwai.m_shuju.m_flag); //0为恒温温度，1为探测器温度
//				AinstrRun(ainstr3, m_ziwai.m_shuju.m_nongdu[2], m_ziwai.m_shuju.m_xishu[2], m_ds.m_wendu[0]); //0为恒温温度，1为探测器温度
				timerPWM.outPWM(0,ainstr1.getCanshuZhi(106));
//				timerPWM.outPWM(1,ainstr2.getCanshuZhi(106));
//				timerPWM.outPWM(2,ainstr3.getCanshuZhi(106));
			}
